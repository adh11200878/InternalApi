...
namespace InternalApi.Helper
{
    public class FileHelper
    {
        ...
        private readonly string[] _ImgAllowedExtensions;//IMG副檔名
        private readonly string[] _ImgAllowedMimeTypes;//IMG MIME TYPE限制
        public FileHelper(IConfiguration configuration, IWebHostEnvironment env)
        {
            ...
            _ImgAllowedExtensions = _configuration.GetSection("FileUploadOptions:ImgAllowedExtensions").Get<string[]>() ?? Array.Empty<string>();
            _ImgAllowedMimeTypes = _configuration.GetSection("FileUploadOptions:ImgAllowedMimeTypes").Get<string[]>() ?? Array.Empty<string>();
        }
        #endregion

        #region 照片上傳
        /// <summary>
        /// 照片共用函數 (回傳false message為錯誤訊息 | 回傳ture mesaage是檔案名稱)
        /// </summary>
        /// <param name="file">檔案</param>
        /// <param name="newFileName">新檔案名稱</param>
        /// <returns></returns>
        public async Task<(bool isSuccess, string messageOrFileName)> UploadImgAsync(IFormFile file, string newFileName = "")
        {
            ...
            if (_ImgAllowedExtensions == null || !_ImgAllowedExtensions.Contains(extension))
            ...
            if (_ImgAllowedMimeTypes == null || !_ImgAllowedMimeTypes.Contains(mimeType))
            {
        ...
        #region 單筆檔案上傳
        /// <summary>
        /// 單筆檔案共用函數 (回傳false message為錯誤訊息 | 回傳ture mesaage是檔案名稱)
        /// </summary>
        /// <param name="file">檔案</param>
        /// <param name="newFileName">新檔案名稱</param>
        /// <returns></returns>
        public async Task<(bool isSuccess, string messageOrFileName)> UploadFileAsync(IFormFile file,string newFileName = "")
        {
            if (file == null || file.Length == 0)
                return (true, ""); // 當作允許的情境處理，不是錯誤
